...
export default function FGStockEntryManual() {
  ...
  const submit = async () => {
    ...
    const payload = rows
      .filter(r => r.material_name && r.batch_no)
      .map(r => ({
    ...
    if (!payload.length) {
      alert('Material Name + Batch No required');
      return;
    }
    ...
    alert('FG Manual Stock Entry Saved');
            ...
            <tr key={i}>
              ...
              <td>
                <input
                  value={r.entry_uom}
                  onChange={e =>
                    updateRow(i, { entry_uom: e.target.value })
                  }
                />
              </td>
